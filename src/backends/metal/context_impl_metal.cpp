#include "context_impl_metal.hpp"
...
namespace elemd
{
    ...
    inline ContextImplMetal* getImpl(Context* ptr)
    {
        return (ContextImplMetal*)ptr;
    }
    inline const ContextImplMetal* getImpl(const Context* ptr)
    {
        return (const ContextImplMetal*)ptr;
    }
    ...
    {
        return new ContextImplMetal(window);
    }

    void Context::stroke_rect(float x, float y, float width, float height)
    {
        ContextImplMetal* impl = getImpl(this);
    ...
    void Context::draw_text(float x, float y, std::u32string text)
    {
        ...
        ImageImplMetal* img = (ImageImplMetal*)_font->get_image();
    ...
    {
        ContextImplMetal* impl = getImpl(this);
        ImageImplMetal* img = (ImageImplMetal*)image;
    ...
    void Context::set_rect_mask(float x, float y, float width, float height)
    {
        ...
        impl->current_type = ContextImplMetal::SCISSOR;
    ...
    void Context::remove_rect_mask()
    {
        ...
        impl->current_type = ContextImplMetal::SCISSOR_CLEAR;
        impl->draw_call_indices.push_back({(int)impl->storage.size(),
                                           (int)impl->scissor_primitives.size() - 1,
                                           ContextImplMetal::SCISSOR_CLEAR});
    }
    ...
    void Context::draw_frame()
    {
        ...
        //{
        //    if (impl->draw_call_indices[i].type == ContextImplMetal::draw_call_type::COLOR)
        //    {
        ...
        //        GLsizei size = (GLsizei)(batch_length * sizeof(ContextImplMetal::uniform_rect));
        ...
        //    }
        //    else if (impl->draw_call_indices[i].type == ContextImplMetal::draw_call_type::SCISSOR)
        //    {
        //        // std::cout << i + 1 << " CALL TYPE: SCISSOR\n";
        //        ContextImplMetal::scissor_primitive sp =
        ...
        //             ContextImplMetal::draw_call_type::SCISSOR_CLEAR)
        //    {
        ...
        //impl->current_type = ContextImplMetal::UNSET;
    ...
    {
        ContextImplMetal* impl = getImpl(this);
        FontImplMetal* fiv = (FontImplMetal*)font;
        if (!fiv->_uploaded)
        {
            fiv->upload();
            
            ImageImplMetal* img = (ImageImplMetal*)font->get_image();
            if (impl->images.size() > impl->texture_array_size)
            {
                img->_sampler_index = 0;
    ...
    void ContextImplMetal::queue_color_call()
    {
        // Start new color sequence
        if (current_type != COLOR || current_color_call_cnt >= MAX_UNIFORM_RECT_PER_BLOCK_COUNT)
        {
            draw_call_indices.push_back({(int)storage.size() - 1, -1, ContextImplMetal::COLOR});
    ...
    void ContextImplMetal::configure_surface()
    {
    ...
    void ContextImplMetal::create_uniform_buffer()
    {
    ...
    void ContextImplMetal::initialize_resources()
    {
        ...
        buffer[2] = 0;
        buffer[3] = 0;
        dummy = new ImageImplMetal(1, 1, 4, buffer, {});
        dummy->upload();
    }

    void ContextImplMetal::update_viewport(uint32_t width, uint32_t height)
    {
        //if (resizing)
        //    return;
        //resizing = true;
    ...
    ContextImplMetal::ContextImplMetal(Window* window)
    {
        ...
        MetalSharedInfo::getInstance();
        MAX_UNIFORM_RECT_PER_BLOCK_COUNT = MetalSharedInfo::MAX_UNIFORM_BLOCK_SIZE / sizeof(uniform_rect);
        create_shader_programm();
    }

    ContextImplMetal::~ContextImplMetal()
    {
    ...
    void ContextImplMetal::create_vertex_array()
    {
    ...
    void ContextImplMetal::create_shader_programm()
    {
    ...
    void ContextImplMetal::update_uniform_buffer()
    {
        //glBindBuffer(GL_UNIFORM_BUFFER, storageBuffer);
        //GLsizei size = (GLsizei)std::min(storage.size() * sizeof(uniform_rect),
        //                                 (size_t)MetalSharedInfo::MAX_UNIFORM_BLOCK_SIZE);
        //                                 
